...
def histplt (val,bins,title,xlabl,ylabl,xlimt,ylimt,font: dict,tot=10000,scale=0.55,axt=None,missin_rep=True):
    #
    ax1 = axt or plt.axes()
    tot=len(val)
    val=val[~np.isnan(val)]
...
def cplotmatrix(df,font: dict,alpha=0.8,marker='ro',missin_rep=True,loc=False,scale=1.25):
    Samp=df.values
        ...
        for i in range(nvar):
            ...
            if (i==j):
                ...
                histplt (x_,15,title=title,
                     xlabl=xlabl,ylabl=ylabl,xlimt=xlimt,ylimt=(0,0.4),axt=ax1,missin_rep=missin_rep,font=font,scale=scale)
            elif(i<=j):
                ax1 = plt.subplot(nvar,nvar,ir)
                title=''
                xlabl=None;ylabl=None
                ...
                CrossPlot(x_,y_,title=title,
                          xlabl=xlabl,ylabl=ylabl,loc=loc_,xlimt=(np.nanmin(x_),np.nanmax(x_)),
                          ylimt=(np.nanmin(y_),np.nanmax(y_)),axt=ax1,alpha=alpha,marker=marker,font=font,scale=scale)
                ik+=1
